...
import { LayoutDashboard, Briefcase, FileText, CheckSquare, Calendar, Users, Shield, X, LogOut, DollarSign, Contact } from "lucide-react";
...
export default function PortalSidebar({ open, onClose }) {
  ...
  const isAdmin = !!permissions?.can_manage_users;
  const items = isAdmin ? [...filteredNavItems, adminItem] : filteredNavItems;
    ...
    <>
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-[#080808] border-r border-[#1A1A1A] flex flex-col transition-transform duration-300 ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        <div className="h-16 flex items-center justify-between px-6 border-b border-[#1A1A1A]">
          ...
          <button onClick={onClose} className="lg:hidden text-[#F5F5F3]/40"><X size={18} /></button>
        </div>
